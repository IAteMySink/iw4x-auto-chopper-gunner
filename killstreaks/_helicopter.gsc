// IAteMySink Autonomous Chopper Gunner Mod
...
//	precacheitem( "cobra_turret_minigun_mp" ); // Possibly Needed? not sure.
	...
	level.chopper_turretClipSize	= 10000; // Chopper Gunner clip size (Realistically infinite)
...
tryUseHelicopter( lifeId, heliType )
{
	...
	if ( (!isDefined( heliType ) || (heliType == "flares") || (heliType == "minigun")) && isDefined( level.chopper ) )
	{
	...
	if ( isDefined( heliType ) && heliType == "null" ) // instead of "minigun". this stops from trying to start the controllable killstreak sequence for the Chopper Gunner.
	{
	...
	self switchToWeapon( self getLastWeapon() ); // Close the damn computer!!
	return true;
}
...
//	}
	...
	{
		case "minigun":
			chopper thread attack_targets_chopper(); // New type of target attacking
			chopper thread heli_targeting();
			chopper heli_fly_simple_path( startNode );
			thread teamPlayerCardSplash( "used_helicopter_minigun", owner );
			chopper thread heli_leave_on_timeout( 60.0 ); // 60 Seconds now. Feel free to revert to 40
			if ( attackAreas.size )
...
fire_missile_chopper( sMissileType, iShots, eTarget ) // New type for Chopper Gunner Missiles
{
	if ( !isdefined( iShots ) )
		iShots = 1;
	assert( self.health > 0 );
	...
	weaponName = undefined;
	weaponShootTime = undefined;
	defaultWeapon = "cobra_player_minigun_mp";
	tags = [];
	switch( sMissileType )
	{
		case "ffar":
...
attack_targets_chopper() // Alt. Version for Chopper Gunner
{
	//self thread turret_kill_players();
	self thread attack_primary_chopper();
	self thread attack_secondary_chopper();
}
...
missile_support_chopper( target_player, rof )
{
	self endon( "death" );
	self endon( "crashing" );
	self endon( "leaving" );	
	...
	if ( isdefined( target_player ) )
	{
		if ( level.teambased )
		{
			if ( isDefined( target_player.owner ) && target_player.team != self.team )
			{
				self fire_missile_chopper( "ffar", 1, target_player );
				self notify( "missile fired" );
			}				
		}
		else
...
// mini-gun with missile support, Chopper Gunner Version. (Missiles are in attack_secondary_chopper)
attack_primary_chopper()
{
	self endon( "death" );
	self endon( "crashing" );
	self endon( "leaving" );
	...
	while ( 1 )
	{
		...
		weaponShootTime = weaponFireTime( "cobra_player_minigun_mp" );
		
		convergenceMod = 1;
		shotsSinceLastSighting = 0;

//		self playLoopSound( "weap_cobra_20mm_fire_npc" );
		for ( i = 0; i < level.chopper_turretClipSize; i++ )
		{
			self setVehWeapon( "cobra_player_minigun_mp" );
			self fireWeapon( "tag_flash" );

			if ( i < level.chopper_turretClipSize - 1 )
				wait weaponShootTime;
